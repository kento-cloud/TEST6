...
import { insertMockStepContent } from "@/lib/ai/mock-fallback"
...
export async function POST(
  ...
  try {
    ...
    if (isConfigError) {
      const mockValue = await insertMockStepContent(id, step as "summary" | "chapters" | "article" | "tags")
      return NextResponse.json({ status: "done", step, mock: true, result: mockValue })
    }
